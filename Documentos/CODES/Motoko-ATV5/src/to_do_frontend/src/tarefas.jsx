...
function tarefas() {
  ...
  const [totalTarefasEmAndamento, settotalTarefasEmAndamento] = useState(0);
  ...
  async function consultarTarefas() {
    const tarefasAtualizadas = await to_do_backend.getTarefas();
    setTarefas(tarefasAtualizadas);
    atualizartotalTarefasConcluidas(tarefasAtualizadas);
    atualizartotalTarefasEmAndamento(tarefasAtualizadas);
  }
  // funções para atualizar os totais
  function atualizartotalTarefasConcluidas(tarefas) {
    settotalTarefasConcluidas(tarefas.filter(ta => ta.concluida).length);
  }
  
  function atualizartotalTarefasEmAndamento(tarefas) {
    settotalTarefasEmAndamento(tarefas.filter(ta => !ta.concluida).length);
  }
  ...
  async function alterar(id, categoria, descricao, urgente, concluida) {      
    ...
    // Atualiza o estado localmente antes de buscar no backend
    setTarefas((tarefas) =>
        tarefas.map((ta) =>
            ta.id === id ? { ...ta, categoria, descricao, urgente, concluida } : ta
        )
    );

    consultarTarefas(); // Atualiza a lista de tarefas com os dados do backend
}
  ...
  return (
    ...
    <main class="mt-[30px] mx-[30px]">
        ...
        <div class="text-right font-bold text-gray-700 dark:text-gray-300">TOTAL: {totalTarefasEmAndamento}</div>
        <br/>
        ...
        <div class="text-right font-bold text-gray-700 dark:text-gray-300">TOTAL: {totalTarefasConcluidas}</div>
